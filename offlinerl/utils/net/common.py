...
class Net(nn.Module):
    ...
    def __init__(
        ...
        if dueling is None:
            if action_shape and not concat:
                print("action_shape and not concat common.py --net")
                model += [nn.Linear(hidden_layer_size, np.prod(action_shape))]
        else:  # dueling DQN
            print("dueling DQN common.py --net")
    ...
    def forward(
        ...
        if self.dueling is not None:  # Dueling DQN
            print("self.dueling --net")
            q, v = self.Q(logits), self.V(logits)
            logits = q - q.mean(dim=1, keepdim=True) + v
        if self.softmax:
            print("self.softmax --net")
